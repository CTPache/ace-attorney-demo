...
- Core responsibilities:
  ...
  - `js/save-load.js` and `js/save-load/helpers.js`: browser `localStorage` save/load orchestration, snapshot and restore helpers, and resume flow.
...
- Keep save/load changes synchronized across `js/save-load.js` and `js/save-load/helpers.js`. If new state must persist, update both the snapshot and restore path, then verify the title-screen and config-menu entry points still work.
...
- If you touch save/load, verify saving from active gameplay and loading from both the config menu and the title screen, including after a language change.
